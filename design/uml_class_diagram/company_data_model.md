...
| Shareholder returns | `dividendPerShare`, `dividendsAgainstFreeCashFlow` (the inputs of each share), `buybacksNetOfStaffShares` (both sides of each year, so a 10-K stays in the index when no net exists), `shareCount`, `totalShareholderYield` (the reported inputs of each yield: dividends paid, share repurchases, proceeds from stock plans, diluted shares and the year-end price). Each block names no filing until the masthead, the financials and the Shareholder returns section load, since the tab draws no card before then |
