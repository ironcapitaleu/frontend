          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="glass-strong p-6 rounded-xl">
                <h3 className="text-sm font-semibold text-tertiary mb-2">Market Cap</h3>
                <p className="h3 text-primary">${formatNumber(selectedCompany.marketCap)}</p>
              </div>
              
              <div className="glass-strong p-6 rounded-xl">
                <h3 className="text-sm font-semibold text-tertiary mb-2">Sector</h3>
                <p className="h3 text-primary">{selectedCompany.sector}</p>
              </div>
              
              <div className="glass-strong p-6 rounded-xl">
                <h3 className="text-sm font-semibold text-tertiary mb-2">Industry</h3>
                <p className="h3 text-primary">{selectedCompany.industry}</p>
              </div>
              
              {selectedCompany.employees && (
                <div className="glass-strong p-6 rounded-xl">
                  <h3 className="text-sm font-semibold text-tertiary mb-2">Employees</h3>
                  <p className="h3 text-primary">{selectedCompany.employees.toLocaleString()}</p>
                </div>
              )}
              
              {selectedCompany.founded && (
                <div className="glass-strong p-6 rounded-xl">
                  <h3 className="text-sm font-semibold text-tertiary mb-2">Founded</h3>
                  <p className="h3 text-primary">{selectedCompany.founded}</p>
                </div>
              )}
              
              <div className="glass-strong p-6 rounded-xl">
                <h3 className="text-sm font-semibold text-tertiary mb-2">Headquarters</h3>
                <p className="h3 text-primary">{selectedCompany.headquarters}</p>
              </div>
            </div>
              ...
              <div className="glass-strong p-6 rounded-xl">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold">${formatNumber(selectedCompany.marketCap)}</span>
                  </div>
                  <div className="flex justify-between items-center">
              ...
              <div className="glass-strong p-6 rounded-xl">
                ...
                <div className="space-y-4">
                  <div className="pb-3 border-b border-glass-border">
                    <h4 className="font-semibold text-primary mb-1">Strong Q4 Earnings Beat Expectations</h4>
                    <p className="text-secondary text-sm">Company reports record quarterly revenue...</p>
                    <span className="text-xs text-tertiary">2 hours ago</span>
                  </div>
                  <div className="pb-3 border-b border-glass-border">
                    <h4 className="font-semibold text-primary mb-1">New Product Launch Announcement</h4>
                    <p className="text-secondary text-sm">CEO unveils next-generation technology...</p>
                    <span className="text-xs text-tertiary">1 day ago</span>
                  </div>
                  <div>
                    <h4 className="font-semibold text-primary mb-1">Analyst Upgrade to Buy Rating</h4>
                    <p className="text-secondary text-sm">Major investment bank raises price target...</p>
        ...
        <div className="glass p-12 rounded-xl text-center">
          <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-glass-bg flex items-center justify-center">
            <svg className="w-8 h-8 text-tertiary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m-3 16a9 9 0 119-9 9 9 0 01-9 9z" />
            </svg>
          </div>
          <h3 className="h3 mb-2">No results found</h3>
          <p className="text-secondary">Try searching for a different company name or symbol</p>
        </div>
      )}
      ...
      {!searchTerm && (
        <div className="glass p-12 rounded-xl text-center">
          ...
          <p className="text-secondary mb-6">
            Enter a company name or stock symbol to get detailed company information, 
            financial data, and recent news.
          </p>
          <div className="flex flex-wrap gap-2 justify-center">
            <button onClick={() => setSearchTerm('AAPL')} className="btn btn-glass text-sm">Apple</button>
            <button onClick={() => setSearchTerm('TSLA')} className="btn btn-glass text-sm">Tesla</button>
            <button onClick={() => setSearchTerm('NVDA')} className="btn btn-glass text-sm">NVIDIA</button>
          </div>
        </div>
      )}
    </div>
