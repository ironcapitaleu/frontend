...
When you establish a new convention, human or AI, decide where it sits on this
ladder and push it as high as it goes:

1. **CI gate** — a check catches the mistake mechanically. This is true
   poka-yoke and the strongest rung. Prefer it whenever a script can decide pass
   or fail (the story-presence gate is one example).
...
3. **Documentation (this file)** — the rule is recorded here and has to be
   remembered. This is the weakest rung. AI drift is statistical, and a rule that
   lives only in prose erodes without anyone noticing.

A convention parked at rung 3 that a script could enforce at rung 1 is a latent
regression. When you add a rule here, take it to the highest rung it reaches, and
state why if you stop short.
