...
Components and pages carry two kinds of coverage, and each kind has one home.
The split below keeps stories and unit tests from proving the same thing twice.
[TESTING.md](./TESTING.md) holds the full doctrine. This section states the
division of labor that governs every UI change.

### Stories are the contract for visual components

Every component in `src/components/ui` ships a story for each meaningful variant
and state. Stories run as render tests in the Vitest Storybook project, so a
story *is* a render test. A component folder without a `.stories.tsx` file fails
CI through the story-presence gate (`npm run check:stories`).

Interaction behavior (open a dropdown, select an item, toggle a switch) lives in
story `play` functions. The documented behavior is the tested behavior. Do not
write a `renders without crashing` unit test, because the story already proves
the component renders.

### Unit tests are for logic, not rendering

`.test.tsx` and `.test.ts` files cover logic: hooks, utilities, conditional
branches, edge cases, and error states. They follow the Arrange-Define-Act-Assert
shape with exactly one `expect` per test and `should … when …` names (see
...
Tests never reach a real external service (auth, network, storage, a vendor SDK).
The convention is dependency injection behind an app-owned port, not vendor
mocking. See **Dependency Injection & Ports** below and [TESTING.md](./TESTING.md)
§5 for the port, the real adapter, the named fakes, and the custom `render()`. A
test injects a fake through the provider. There is nothing to `vi.mock`.

---

## The Enforcement Ladder (Poka-Yoke)

When you establish a new convention, human or AI, decide which of these three
levels it sits at, and take it to the highest level you can:

1. **CI gate** — a check catches the mistake mechanically. This is the strongest
   level and true poka-yoke. Prefer it whenever a script can decide pass or fail
   (the story-presence gate is one example).
2. **Scaffold or skill** — the right structure is generated by default, so the
   common path is correct without anyone recalling the rule (the
   `component-scaffold` skill is one example).
3. **Documentation (this file)** — the rule is recorded here and has to be
...
Every public page is a top-level module under `src/pages/`. The registry in
`src/pages/sitemap.ts` (`SITEMAP_MEMBERS`) is the single source of truth for the
sitemap: the sitemap page (`src/pages/SitemapPage.tsx`) renders its links from
it, and the sitemap XML (`public/sitemap.xml`) is checked against it. Correct
sitemap data matters for SEO and for readers who navigate the site by it.

**To add a public page:** put its module at the top level of `src/pages/`, add
an entry to `SITEMAP_MEMBERS`, and add its URL to `public/sitemap.xml`.

**For a page that must NOT appear in the sitemap** (an auth-only page, or the
catch-all 404): put it in `src/pages/internal/`. Discovery does not look into
that subfolder, so it stays off the sitemap.

The sitemap-consistency gate is a unit test (`src/pages/sitemap.test.ts`, run by
`npm run test:ci`), so it needs no separate CI step. `discoverPageModules()`
walks the top level of `src/pages/` and the test locks three things together:
the discovered page modules equal the registry, and the registry paths equal the
`<loc>` URLs in `public/sitemap.xml`. Adding a page to the folder without
registering it, or registering one that does not exist, or a sitemap XML that
drifts from the registry, all fail the test. This is why it enforces the sitemap
is correct, not merely that someone remembered to touch the files.
...
- **New or changed component means its story is updated. New logic means a unit test covers
  it.** A `src/components/ui` folder with no `.stories.tsx` fails the story-presence gate
  (`npm run check:stories`)
